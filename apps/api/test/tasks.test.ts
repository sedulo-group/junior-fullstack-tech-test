...
test("lists seeded tasks and creates, reads, updates and deletes a task", async () => {
  ...
  const updated = await request(`/tasks/${task.id}`, "PATCH", {
    status: "done",
  });
  assert.equal(updated.status, 200);
  assert.equal((await updated.json()).status, "done");
  assert.equal(
    (await (await request(`/tasks/${task.id}`)).json()).status,
    "done",
  );
...
test("rejects blank, missing, non-string, oversized and unexpected input", async () => {
  ...
  assert.equal(
    (await request("/tasks/1", "PATCH", { status: "invalid" })).status,
    400,
  );
  assert.equal((await request("/tasks/1", "PATCH", {})).status, 400);
  assert.equal(
    (await request("/tasks/1", "PATCH", { status: "done", title: "Changed" }))
      .status,
    400,
  );
});
...
test("returns 404 for reads, updates and deletes of unknown tasks", async () => {
  ...
  assert.equal(
    (await request("/tasks/missing", "PATCH", { status: "done" })).status,
    404,
  );
