...
test("filter, create, navigate, update, reload and delete", async ({
  page,
}) => {
  ...
  await page
    ...
    .getByRole("link", { name: "Add task" })
    .click();
  await expect(page.getByLabel("Title", { exact: false })).toBeFocused();
  await page
    .getByLabel("Title", { exact: false })
    .fill("Interview browser task");
  await page
    .getByLabel("Description")
    .fill("Created through the real NestJS API.");
  await acceptCreatePrompts(page);
  await expect(
    page.getByRole("heading", { name: "Interview browser task" }),
  ).toBeVisible();
  ...
  await page
    .getByRole("link", { name: "Interview browser task", exact: true })
    .click();
  ...
  await expect(page.getByText("3 tasks shown · 1 of 3 complete")).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Interview browser task", exact: true }),
  ).toHaveCount(0);
...
test("failed create keeps form values and allows a retry", async ({ page }) => {
  ...
  await page.route("**/api/tasks", (route) =>
    route.fulfill({ status: 500, json: { message: "Could not save task" } }),
  );
  await acceptCreatePrompts(page);
  await expect(page.getByRole("main").getByRole("alert")).toContainText(
    "Could not save task",
...
test("failed status update keeps the saved status, and missing tasks explain the error", async ({
  page,
}) => {
  await page.goto("/tasks/1");
  await expect(page.getByLabel("Status", { exact: true })).toHaveValue("todo");
  await page.route("**/api/tasks/1", (route) =>
    route.fulfill({ status: 500, json: { message: "Could not save status" } }),
  );
  await page.getByLabel("Status", { exact: true }).selectOption("done");
  await page.getByRole("button", { name: "Save status" }).click();
  await expect(page.getByRole("main").getByRole("alert")).toContainText(
...
test("mobile layout fits and all three pages render", async ({ page }) => {
  ...
  for (const path of ["/tasks", "/tasks/new", "/tasks/1"]) {
    ...
    await page.screenshot({
      path: `test-results/mobile-${path.split("/").pop()}.png`,
      fullPage: true,
    });
    expect(
      await page.evaluate(
  ...
  await expect(page.getByText("3 tasks shown · 1 of 3 complete")).toBeVisible();
  await page.screenshot({
    path: "test-results/desktop-tasks.png",
    fullPage: true,
  });
});

test("summary counts agree with task statuses", async ({ page }) => {
  await page.goto("/tasks");
  const overview = page.getByRole("region", { name: "Task overview" });
  await expect(overview).toBeVisible();
  for (const label of ["To do", "In progress", "Done"]) {
    await expect(overview.getByText(label, { exact: true })).toBeVisible();
  }
  await expect(overview.getByText("1", { exact: true })).toHaveCount(3);
});

for (const cancelAt of [1, 2]) {
  test(`cancelling creation at confirmation ${cancelAt} preserves the draft`, async ({
    page,
  }) => {
    await page.goto("/tasks/new");
    await page
      .getByLabel("Title", { exact: false })
      .fill("Keep this unsaved task");
    let prompts = 0;
    let writes = 0;
    ...
    page.on("dialog", async (dialog) => {
      prompts += 1;
      if (prompts === cancelAt) await dialog.dismiss();
      else await dialog.accept();
    });
    await page.getByRole("button", { name: "Create task" }).click();
    await expect(page.getByLabel("Title", { exact: false })).toHaveValue(
      "Keep this unsaved task",
    );
    await expect(
      page.getByRole("button", { name: "Create task" }),
    ).toBeEnabled();
    expect(prompts).toBe(cancelAt);
